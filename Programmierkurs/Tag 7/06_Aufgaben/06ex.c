...
Canvas swatch_red_green(Canvas c) {
    ...
    for(int x = 0; x < c_width; x++){
        for(int y = 0; y < c_height; y++){
            canvas_set_r(c, x, y, normalize(x, c_width - 1));
            canvas_set_g(c, x, y, normalize(y, c_height - 1));
        }
    }
    return c;
}
...
Canvas swatch_rgb(Canvas c, ColorChannel channel_x, ColorChannel channel_y) {
    ...
    for(int x = 0; x < c_width; x++){
        for(int y = 0; y < c_height; y++){
            canvas_set_channel(c, x, y, channel_x, normalize(x, c_width - 1));
            canvas_set_channel(c, x, y, channel_y, normalize(y, c_height - 1));
        }
    }
    return c;
}
...
Canvas canvas_set_rgb_struct(Canvas c, int x, int y, RGB color) {
    canvas_set_channel(c, x, y, Red, color.r);
    canvas_set_channel(c, x, y, Green, color.g);
    canvas_set_channel(c, x, y, Blue, color.b);
    return c;
}
...
*/
Canvas set_pixel_gray(Canvas c, int x, int y){
    // Save pixel color in an RGB struct
    RGB pixel_color;
    pixel_color.r = canvas_get_r(c, x, y);
    pixel_color.g = canvas_get_g(c, x, y);
    pixel_color.b = canvas_get_b(c, x, y);

    // Set RGB struct into corresponding gray color
    pixel_color = rgb_to_gray(pixel_color);

    // Set pixel to gray color
    canvas_set_rgb_struct(c, x, y, pixel_color);
